...
class AdvertisementController extends Controller
{
    ...
    public function store(Request $request)
    {
    	$request->validate([
            ...
            'size' => 'required|in:728x90,300x250',
        ]);
        if($request->type == 1){
            $request->validate([
        ...
        if($request->adimage){
            ...
            try {
                $advr->adimage = uploadImage($request->adimage, $path);
            } catch (\Exception $exp) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->adimage){
            ...
            try {
                $advr->adimage = uploadImage($request->adimage,$path,null,$advr->adimage);
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Image could not be uploaded.'];
                return back()->withNotify($notify);
            }
    ...
    public function delete(Request $request)
    {
        ...
        $ads =  Advertise::findOrFail($request->id);
        $ads->delete();
        $notify[]=['success','Advertisement delete successfully'];
        return back()->withNotify($notify);
    }
}
